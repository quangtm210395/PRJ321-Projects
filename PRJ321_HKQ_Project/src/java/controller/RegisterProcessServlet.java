...
public class RegisterProcessServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!Checker.checkName(name)) {
            message += "Name must contains only character and white space! <br>";
        } else name = Checker.toNormalForm(name);
        ...
        boolean  valid;
        try {
            valid = Checker.isRegisSuccessful(name, username, password, re_password);
        } catch (Exception ex) {
            ex.printStackTrace();
            valid = false;
        }
